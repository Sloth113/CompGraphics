...
uniform vec3[4] Ids; // diffuse light colour 
uniform vec3[4] Iss; // specular light colour 
uniform vec3[4] LightDirections;
...
void main() {
	...
	for(int i =0; i < 4; i++){
		...
		vec3 L = normalize(-LightDirections[i]);//-L 
		...
		//Colour?
		vec3 ambient = Ia * Ka;    
		vec3 diffuse = Ids[i] * Kd * lambertTerm ;
		vec3 specular = Iss[i] * Ks * specularTerm;
		
		colour += vec3( ambient + diffuse + specular) ;
	}
